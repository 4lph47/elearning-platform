...
// Página própria (antes vivia dentro de app/students/[id]/page.tsx) — igual
// ao padrão já usado para Analytics (app/dashboard/analytics), só o dono vê
// isto, acessível pelo botão "Gerir revendas" no perfil público.
export default async function StudentResalePage() {
  const session = await getServerSession(authOptions);
  if (!session) redirect("/login?callbackUrl=/dashboard/resale");

  const userId = session.user.id;

  const [completedCourses, ownListings, ownBundles] = await Promise.all([
    prisma.enrollment.findMany({
      where: { userId, completedAt: { not: null }, course: { resaleMinCommission: { not: null } } },
