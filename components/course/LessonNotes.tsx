        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {filteredNotes.map((note) => {
            ...
            return (
              <div
                key={note.id}
                className={`group relative flex flex-col overflow-hidden rounded-lg border bg-white hover:shadow-md dark:bg-white/5 ${
                  isFromCurrentLesson
              ...
              >
                {isFromCurrentLesson && (
                  <div className="absolute right-2 top-2 rounded bg-blue-600 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white">
                    Aula atual
                  </div>
                )}
                <div className="flex-1 space-y-2 p-3">
                  <h4 className="line-clamp-2 pr-16 text-sm font-semibold text-slate-900 dark:text-white">{note.title}</h4>
                  <p className="line-clamp-3 text-xs text-slate-600 dark:text-slate-300">{note.content}</p>
                ...
                <div className="flex items-center gap-1 border-t border-slate-100 bg-slate-50 p-2 dark:border-white/5 dark:bg-white/5">
                  <button
                    onClick={() => openEditor(note)}
                    className="flex flex-1 items-center justify-center gap-1.5 rounded px-2 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/10"
                  >
                    <Edit size={12} />
                    Editar
                  </button>
                  <button
                    ...
                    className="flex flex-1 items-center justify-center gap-1.5 rounded px-2 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10"
                  >
                    <Trash2 size={12} />
                    Remover
                  </button>
