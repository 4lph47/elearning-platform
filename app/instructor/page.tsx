    ...
    <div className="min-h-screen bg-black px-4 py-10 sm:px-8">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-white">Área de instrutor</h1>
            <p className="mt-1 text-sm text-slate-400">Gere os teus cursos e acompanha o desempenho.</p>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/instructor/profile" className="text-sm font-medium text-slate-400 hover:text-white">
              Perfil público
            </Link>
            <Link
        ...
        {courses.length > 0 && (
          <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <div className="rounded-xl border border-white/10 bg-slate-950 p-4">
              ...
              <p className="mt-2 text-2xl font-bold text-white">{avgRating !== null ? avgRating.toFixed(1) : "—"}</p>
        ...
        {courses.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-slate-950 p-10 text-center">
            <p className="text-slate-400">Ainda não criaste nenhum curso.</p>
            <Link
              href="/instructor/courses/new"
            ...
            {courses.map((course) => {
              ...
              return (
                <Link key={course.id} href={`/instructor/courses/${course.id}`}>
                  <div className="flex items-center gap-4 rounded-xl border border-white/10 bg-slate-950 p-4 transition-colors hover:border-white/20">
                    {course.thumbnailUrl ? (
                      ...
                      <div className="flex h-16 w-24 shrink-0 items-center justify-center rounded-md bg-slate-900 text-lg font-bold text-slate-600">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-400">
                        ...
                        {course.ratingCount > 0 && (
                          <span className="flex items-center gap-1">
                            <Star size={12} className="fill-blue-400 text-blue-400" /> {course.rating.toFixed(1)} (
                            {course.ratingCount})
                          </span>
                        )}
                        <span className="flex items-center gap-1">
