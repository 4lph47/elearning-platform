...
export function LessonNotes({ lessonId }: { lessonId: string }) {
  ...
  if (isCreating || editingNote) {
    const editorContent = (
      <>
        {/* Desktop: renderiza inline normalmente */}
        <div className="hidden md:block">
          ...
          <div className="space-y-3">
            <input
              ...
              maxLength={200}
              className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-white/10 dark:bg-white/5 dark:text-white dark:placeholder:text-slate-500"
            />
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={saveNote}
                disabled={saving}
                className="flex items-center justify-center gap-1.5 rounded-full bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50"
              >
                <Save size={14} />
              ...
              <button
                onClick={closeEditor}
                className="rounded-full px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/5"
              >
                Cancelar
              </button>
            </div>
        ...
        {/* Mobile: Portal para cobrir TUDO - renderizado diretamente no body */}
        {mounted && createPortal(
          <div className="fixed inset-0 z-[99999] flex flex-col bg-white dark:bg-neutral-900 md:hidden">
            {/* Header */}
            <div className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 py-3 dark:border-white/10 dark:bg-neutral-900">
            ...
            {/* Mensagem de erro */}
            {error && (
              <div className="mx-4 mt-3 shrink-0 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600 dark:bg-red-900/20 dark:text-red-400">
                {error}
              </div>
            )}
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-3 p-4">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <button
                  onClick={closeEditor}
                  className="flex-1 rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-200 dark:bg-white/5 dark:text-slate-300 dark:hover:bg-white/10"
                >
                  Cancelar
                </button>
              </div>
