...
const SORT_OPTIONS = [
  ...
  { value: "recent", label: "Inscrição mais recente" },
] as const;
...
// Igual ao InstructorCourseGrid (mesma busca/filtros/grelha), mas sobre os
// cursos em que o aluno está inscrito, não os que dá — sem ordenar por
// preço (já são todos dele) e com texto de estado vazio próprio.
export function StudentCourseGrid({
  ...
  return (
    <div>
          ...
          <div className="relative order-1 flex w-full gap-2 sm:order-2 sm:w-auto sm:max-w-xl">
            ...
            >
              <input
                ...
                placeholder={`Procurar nos cursos de ${studentFirstName}...`}
      ...
      {courses.length === 0 ? (
        <p className="text-slate-500 dark:text-slate-400">Ainda não está inscrito em nenhum curso.</p>
      ) : filtered.length === 0 ? (
